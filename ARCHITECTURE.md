...
shader evaluates a finite solar disc, alpha/Glass-aware visibility, a time-varying
Preetham/Perez sky, and up to
...
MIS combines the two techniques without double-counting. The single code-owned yard
contains 21 small emissive placements / 42 triangles whose common intensity follows
time of day: zero in daylight, full at night, and crossfaded through civil twilight.
An acceptance-only linear-HDR reference mode freezes the scene,
...
regressions.
P4 replaces the fixed sun/gradient with one time-of-day authority. A fixed
46.5-degree latitude and summer-solstice declination produce the sun direction;
turbidity-three Preetham xyY/Perez coefficients are computed on the CPU and mirrored
in Slang, with a continuous fade through civil twilight into the authored night
gradient. The sun is a uniform 0.27-degree cone whose radiance integrates back to the
published direct-normal irradiance. NEE and BSDF hits of that disc use complementary
power-heuristic weights while the exact zero-angle configuration retains the old
delta estimator. The same daylight blend inversely drives the permanent yard emitter
set, so both visible emission and NEE energy turn on together without changing scene
geometry. The yard defaults to noon; `--time <hours>` freezes any acceptance time,
and interactive play advances one in-game minute per real second without changing the
192-byte frame-record size or bindings. The pre-consolidation fixed 32+256 fully
configured yard capture at 13:30 recorded a 2.686 ms trace-only
median on the RTX 5070 Ti / 595.71.05 system. Noon, civil-twilight, and full-night
captures completed with distinct deterministic hashes; plain, GPU-assisted, and
synchronization validation were clean across both frame slots.
After consolidating day/night into the one permanent-emitter yard, the deterministic
noon capture proof records frame-7 hash `0x011e175c7a0e3d63`; the midnight
MIS/NEE/BSDF proof still agrees in every pinned region.
P5 gives the existing view-state scalars their final meaning: a pinned fixed-seed
permutation visits every 4×4 subpixel cell once per 16 global sample entries—16 frames
at the default 1 SPP—shared by all pixels and separate from the per-pixel PCG path
stream. The push block's final
word now selects 1, 2, 4, 8, or 16 independent full paths per pixel. Raygen assigns them
consecutive entries in that same global jitter sequence, seeds each path independently,
and averages linear radiance before tonemapping; F5 changes the setting interactively
and `--spp` fixes it for interactive, capture, or reference runs. One SPP preserves
P5's original jitter and RNG sequence. The BSDFs now add a
sampled multiple-scattering return lobe from pinned roughness-energy fits,
closing the energy lost by masked rough GGX microfacets. A reference-only `--furnace`
scene places white Dielectric, Metal, and Glass boxes at low/medium/high roughness in
a constant unit-radiance full-sphere environment with no sun or emitters. Its 256-frame
BSDF proof requires every RGB case to meet the larger of two percent or three standard
errors; all nine ratios measured 0.999–1.009. The same run retained yard estimator
agreement. Two independent fixed-protocol noon captures produced the byte-identical
P5 frame-287 hash `0x422e208234fcf994` and 4.225/4.171 ms trace medians on the RTX
5070 Ti / 595.71.05 system, so the lighting/path-tracing roadmap slice is complete.
The later runtime-SPP control was measured on the same 1920×1080 fully configured noon
yard, RTX 5070 Ti, and 595.71.05 driver with MangoHud disabled. Each entry uses the
fixed 32-warm-up + 256-measured trace-only protocol:

| SPP | Median trace time | Cost versus 1 SPP |
|-----|------------------:|------------------:|
| 1 | 4.143 ms | 1.00× |
| 2 | 11.763 ms | 2.84× |
| 4 | 27.668 ms | 6.68× |
...
Two independent 1-SPP runs produced the byte-identical frame-287 hash
`0xfee2ecea8b9a05ea`. Repeated higher-SPP runs can differ at a sparse set of
one-LDR-step pixels despite their fixed schedules, so the byte-exact deterministic
runtime proof deliberately remains pinned to the default
1-SPP path; the selectable modes are currently a visual/performance control rather
than a widened byte-exact acceptance contract.
...
| [shaders/raytrace.slang](shaders/raytrace.slang), [records.slang](shaders/records.slang), [sampling.slang](shaders/sampling.slang), [bsdf.slang](shaders/bsdf.slang), and [lighting.slang](shaders/lighting.slang) | Six RT entry points and the iterative path loop, with imported ABI records, RNG/sampling/MIS helpers, class-aware dielectric/Metal/Glass BSDF evaluation/sampling/PDF, pinned multiple-scatter energy closure, and analytic-light evaluation | Build-time sources compiled and embedded as one SPIR-V module; Slang depfile tracks every import |
| [src/gallery.hpp](src/gallery.hpp) / [.cpp](src/gallery.cpp) and [src/furnace.hpp](src/furnace.hpp) | File-private asset/placement tables and `loadGalleryScene`, which loads and instantiates the single yard plus its permanent 21-placement emitter set; the same boundary owns P5's reference-only generated nine-case furnace and its fixed measurement contract | Temporary engine-side yard/acceptance policy called by `main()`; it retires when level data has a real owner |
| [src/estimator_mode.hpp](src/estimator_mode.hpp) | Vulkan-free MIS/NEE-only/BSDF-only reference estimator enum | Shared by command-line policy and frame-lighting flag packing |
...
| [third_party/jolt](third_party/jolt) | Trimmed Jolt Physics v5.6.0 library source, CMake support, MIT license, and one documented thread-pool exception-safety patch | Vendored static engine dependency in the sole canonical build |
...
| [src/lighting.hpp](src/lighting.hpp) / [.cpp](src/lighting.cpp) | Vulkan/GLFW-free 80-byte view-only `RaygenPushConstants`, its fixed-seed 4×4 global jitter/sample schedule, the 1/2/4/8/16-SPP setting contract, frame-payload construction, and the C++ known-answer reference for the shader's PCG stream | Plain per-dispatch state composed by `main()`; lighting does not ride this path |
| [src/scene_lighting.hpp](src/scene_lighting.hpp) / [.cpp](src/scene_lighting.cpp) and [src/frame_lighting_layout.hpp](src/frame_lighting_layout.hpp) | Vulkan-free mutable time-of-day/sun/Preetham-sky authority plus the constant full-sphere furnace environment; static-emitter extraction into the exact 64-byte `LightRecord`, power CDF, and 16-byte reverse-lookup records; exact 192-byte `FrameLighting` ABI and flags; selector packing; CPU sky/sun evaluation/sample/PDF/MIS references; and checked dynamic-slot arithmetic | Plain scene state plus immutable emitter tables and per-frame publications; tested inside the canonical build |
...
| [src/capture.hpp](src/capture.hpp) / [.cpp](src/capture.cpp) and [tools/reference_compare.cpp](tools/reference_compare.cpp) | Vulkan-free interactive/capture/reference CLI parsing including fixed SPP selection, checked raw RGBA8 hashing, linear-to-sRGB PPM publication, binary16 conversion, double-precision fixed-region HDR statistics, pairwise estimator gate, furnace energy gate, and fixed-protocol trace-timing median policy | One-shot capture/reference policy used by `main()` after renderer readback; owns no GPU state |
...
| [src/main.cpp](src/main.cpp) | `main()` orchestration, reference-estimator selection, interactive player/free-camera and F5 SPP control, the time-cycling yard, ordinary capture, and frozen-scene HDR reference sampling | Program lifetime |
...
`swap.extent` aspect ratio, and composes it with the successful frame index and a
frame-global offset from the pinned fixed-seed 4×4 schedule into
`RaygenPushConstants` (the extent is read fresh every iteration, so
...
3. Bind the pipeline and descriptor set at `PIPELINE_BIND_POINT_RAY_TRACING_KHR`
   ...
   extent — 1, 2, 4, 8, or 16 independently seeded paths of up to eight surface vertices
   per pixel, averaged in linear HDR (the isolated
   full-sphere furnace raises its truncation cap to 32 for closed rough Glass). The immediately preceding rebuild's post-build
...
`--reference <samples> --estimator <mis|nee|bsdf> [--time <hours>]`. It disables
...
and returns HDR to the `GENERAL` reuse contract. The estimator-reference gallery
profile places only required generated non-Glass assets, keeping the pinned regions
independent of optional local props and excluding P3's deliberately biased
straight-line visibility through intervening Glass; ordinary capture and interactive
play retain the complete configured gallery. `capture.cpp` converts raw binary16
...
agree within the larger of one percent or three combined standard errors. It then runs
`--reference 256 --estimator bsdf --furnace`: nine fixed regions must each reproduce
the unit full-sphere environment within the larger of two percent or three standard
errors. This never feeds history back to the renderer and is not temporal accumulation.
...
- **BDA/ABI probe.** `GeometryRecord` carries 64-bit device addresses for pre-offset
  ...
  unweighted. The finite sun stays a separately sampled uniform-cone technique and is
  never in this selector. Its radiance is `sunIrradiance / sunSolidAngle`; NEE and
  BSDF-hit disc paths receive complementary power-heuristic weights. A zero solid
  angle takes the original delta path without consuming RNG draws or dividing by zero.
...
- **One lighting authority and pinned ABI.** `SceneLighting` owns the mutable analytic
  sun and sky without Vulkan state, validates finite model state and nonnegative
  radiance, normalizes a powered sun, derives selector probabilities, and
  transactionally packs `FrameLighting`. The `alignas(16)` record is exactly 192
  bytes: sun lanes at 0/16,
  sky lanes at 32/48, counts/power/flags at 64, five Perez `float4`s at 80–144,
  `nightZenith` at 160, then daylight blend, emitter scale, and two reserved words at
  176–188. Bit 1
  ...
  Bit 0 selects the Perez-capable evaluator. The five coefficient lanes carry
  Preetham `(x, y, Y)` fits, `skyZenith` carries the matching zenith xyY value, and
  the night-gradient/daylight-blend lanes provide one continuous sky entry point.
  `emitterScale = 1 - daylightBlend` drives both BSDF-hit and NEE emission from the
  yard's permanent emitter records; a zero scale removes the emitter selector branch.
  Estimator bits select MIS, NEE-only, or BSDF-only for offline
  reference capture, while interactive and ordinary capture always publish MIS. A
  powered sky or a positive-scale static-emitter table is a selector branch; when
  both exist they receive equal branch probability,
...
- **Frame-constant ABI.** The 80-byte raygen block begins with the stable camera
  ...
  that prefix plus frame index at 64, camera jitter at 68/72, and the validated
  1/2/4/8/16 sample count at 76 with `static_assert`s on `sizeof` and every `offsetof`.
  P5 fills the jitter scalars for the first sample from a fixed-seed permutation of
  the sixteen 4×4 cell centers; higher modes continue through consecutive cells inside
  the dispatch. Every pixel shares each sample's offset, and neighboring UV-footprint
  rays shift with it. Camera sampling consumes no per-pixel path RNG draw. Raygen
  hashes the global sample sequence with the launch pixel to decorrelate paths while
  keeping fixed-index, fixed-SPP captures repeatable.
...
- **Controls (all GLFW input policy lives in `updateCamera`).** Game-style
  ...
  F5 independently cycles the per-frame sample budget through 1, 2, 4, 8, and 16.
...
4. **Lighting + path tracing.** **Complete: phases P0–P5 plus HDR,
   dielectric/metal/Glass materials, emissive geometry, and multi-bounce transport
   landed** — the shader is
   split into
   ...
   class-aware GGX finite-sun evaluation at every hit, alpha/Glass-aware shadow rays,
   ...
   deliberate defaults. P4 adds the CPU/Slang Preetham/Perez daylight model, a
   fixed-latitude/date time-of-day driver, civil-twilight/night blending, and a
   uniform-cone solar disc with NEE/BSDF MIS while preserving the zero-angle delta
   compatibility path. The one noon-default yard keeps a flat power-weighted CDF over
   42 emitter triangles resident and fades their common intensity with civil twilight.
   P5 adds deterministic 4×4 frame-global camera jitter, runtime-selectable
   1/2/4/8/16-SPP full-path averaging, and a sampled
   multiple-scattering closure for masked rough GGX energy. The frozen midnight
   linear-HDR proof independently runs MIS, NEE-only, and BSDF-only to guard against
   double-counted or missing energy; its companion nine-case white furnace proves all
   three classes at low, medium, and high roughness in untouched HDR. The
   Many-light sampling remains a first-class requirement, with a
