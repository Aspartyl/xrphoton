...
## Step 1 — Promote `findMemoryType` and add a buffer helper
...
usage includes `SHADER_DEVICE_ADDRESS` — every buffer in this feature needs a device
address.
...
- An `AccelerationStructure` struct owning: vertex/index buffers + memory, instance
  buffer + memory, BLAS buffer/memory/handle, TLAS buffer/memory/handle, and the TLAS
  device address (what the RT descriptor set will eventually bind).
...
- Destructor with null guards; declared after `ctx` in `main` so it destructs before
  the device it borrows.
...
`allocateCommandBuffer` and before the render loop:
...
- Submit on the trace queue and wait with a fence (more consistent with the codebase
  than `vkQueueWaitIdle`), then destroy the scratch buffers — they are only needed
  during the build.

## Step 7 — Wire into `main.cpp`

Declare the `AccelerationStructure` owner (after `ctx`, near `swap`), call the build,
`return 1` on failure with a `std::cerr` line, and log success — matching the existing
bring-up logging style. The acceleration structures are swapchain-independent, so the
resize/recreate paths do not change.
